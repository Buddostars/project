...
int main()
{
    ...
    Environment environment(shaderProgram);

    std::cout << "Current Working Directory: " << std::filesystem::current_path() << std::endl;

    // Load the loading screen image as a texture
    GLuint loadingScreenTexture = loadTexture("src/loading-screen-image.png");
    ...
    while (glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && !glfwWindowShouldClose(window))
    {
        ...
        else if (currentState == STATE_GAME)
        {
            ...
            // Draw the environment models
            environment.render(view, projection);
